...
pub const MaterialProcessor = struct {
    ...
    fn processAssimpMaterial(self: *MaterialProcessor, ai_material: *const assimp.aiMaterial, mat_idx: usize) !void {
        ...
        // Extract textures (try ASSIMP first, then auto-discovery)
        try self.extractTextures(ai_material, &gltf_material);
        
        // If no textures found via ASSIMP, try auto-discovery
        if (gltf_material.pbrMetallicRoughness.baseColorTexture == null) {
            try self.autoDiscoverTextures(&gltf_material);
        }
